...
func (service *UserServiceImpl) Create(ctx context.Context, request request.UserCreatedRequest) response.MessageResponse {
	...
	user := domain.UserModel{
		...
		Password:      sql.NullString{String: helper.HashAndSalt(request.Password), Valid: true},
		FirstName:     sql.NullString{String: request.FirstName, Valid: true},
		LastName:      sql.NullString{String: request.LastName, Valid: true},
		UserRoleId:    sql.NullInt64{Int64: request.UserRoleId, Valid: true},
		CompanyId:     sql.NullInt64{Int64: request.CompanyId, Valid: true},
		...
		TokenVersion:  sql.NullInt64{Int64: 0, Valid: true},
		IsVerified:    sql.NullBool{Bool: false, Valid: true},
		IsDelete:      sql.NullBool{Bool: false, Valid: true},
		CreatedAt:     sql.NullTime{Time: time.Now(), Valid: true},
		UpdatedAt:     sql.NullTime{Time: time.Now(), Valid: true},
		CreatedBy:     sql.NullString{String: request.Email, Valid: true},
		UpdatedBy:     sql.NullString{String: request.Email, Valid: true},
	}
...
func (service *UserServiceImpl) ChangePassword(ctx ctx.Context, request request.ChangePasswordRequest, roles ...string) response.MessageResponse {
	...
	user = domain.UserModel{
		Id:            user.Id,
		Email:         user.Email,
		Password:      sql.NullString{String: helper.HashAndSalt(request.NewPassword), Valid: true},
		FirstName:     user.FirstName,
		LastName:      user.LastName,
		UserRoleId:    user.UserRoleId,
		CompanyId:     user.CompanyId,
